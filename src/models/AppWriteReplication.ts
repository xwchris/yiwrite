// modify from https://github.com/marceljuenemann/rxdb-supabase/blob/main/src/supabase-replication.ts
...
export type AppWriteReplicationOptions<RxDocType> = {
  /**
   * The AppWriteClient to replicate with.
   */
  appWriteClient: DatabaseClient
   ...
   */
  pull?: Omit<
    ReplicationPullOptions<RxDocType, AppWriteReplicationCheckpoint>,
    'handler' | 'stream$'
  > & {
    /**
 ...
 */
export interface AppWriteReplicationCheckpoint {
  modified: string
  primaryKeyValue: string | number
}

/**
 * Replicates the local RxDB database with the given AppWrite client.
 *
 * See AppWriteReplicationOptions for the various configuration options. For a general introduction
 * to RxDB's replication protocol, see https://rxdb.info/replication.html
 */
export class AppWriteReplication<RxDocType> extends RxReplicationState<
  RxDocType,
  AppWriteReplicationCheckpoint
> {
  ...
  //   RxReplicationPullStreamItem<RxDocType, AppWriteReplicationCheckpoint>
  // >
  // private realtimeChannel?: RealtimeChannel

  constructor(private options: AppWriteReplicationOptions<RxDocType>) {
  ...
  private async pullHandler(
    lastCheckpoint: AppWriteReplicationCheckpoint | undefined,
    batchSize: number
  ): Promise<
    ReplicationPullHandlerResult<RxDocType, AppWriteReplicationCheckpoint>
  > {
    // let query = this.options.supabaseClient.from(this.table).select()
    const client = this.options.appWriteClient
    const query: string[] = []
    if (lastCheckpoint && lastCheckpoint.modified) {
      ...
      const isNewer = Query.greaterThan(
        this.lastModifiedFieldName,
        lastModified
      )
      // const isSameAge = `${this.lastModifiedFieldName}.eq.${lastModified}`
      const isSameAge = Query.equal(this.lastModifiedFieldName, lastModified)
      // query = query.or(
      ...
      query.push(
        Query.or([
          ...
          Query.and([
            isSameAge,
            Query.greaterThan(this.primaryKey, lastPrimaryKey)
          ])
        ])
      )
    }
    ...
    query.push(
      Query.orderDesc(this.lastModifiedFieldName),
      Query.orderDesc(this.primaryKey),
    ...
    const { documents: data } = await client.listDocuments(this.table, query)
  ...
  private async handleInsertion(
    ...
    const id = (doc as any)[this.primaryKey]
    try {
      await this.options.appWriteClient.createDocument(this.table, id, doc)
      return [] // Success :)
    } catch (error) {
      throw error
    }
  ...
  private async defaultUpdateHandler(
    ...
    Object.entries(row.assumedMasterState!).forEach(([field, value]) => {
      ...
      if (type === 'string' || type === 'number') {
        // TODO
        console.log('query.eq', field, value)
        // query = query.eq(field, value)
      } else if (type === 'boolean' || value === null) {
        console.log('query.is', field, value)
        // query = query.is(field, value)
      } else {
        throw new Error(`replicateSupabase: Unsupported field of type ${type}`)
      }
    })
    await this.options.appWriteClient.updateDocument(this.table, id, doc)
    return true
  }
  ...
  private async fetchByPrimaryKey(
    ...
    //   .limit(1)
    const data = await this.options.appWriteClient.getDocument(
      this.table,
      primaryKeyValue
    )
    // if (error) throw error
    if (!data) throw new Error('No row with given primary key')
    return this.rowToRxDoc(data)
  }
  ...
  private rowToRxDoc(row: any): WithDeleted<RxDocType> {
    ...
    // 删除非必要的appWrite字段
    delete newData['$id']
    delete newData['$databaseId']
    delete newData['$collectionId']
    delete newData['$permissions']
    delete newData['$createdAt']
    delete newData['$updatedAt']

    return newData as WithDeleted<RxDocType>
  }

  private rowToCheckpoint(row: any): AppWriteReplicationCheckpoint {
    return {
